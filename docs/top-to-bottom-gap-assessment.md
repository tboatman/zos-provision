...
| G-018 | Configuration unwinding is conceptual only | The hardest vendor problem is installed-state discovery and local override attribution | Vendor lifecycle defines inventory, attribution, normalization passes; [BMC Read-Only Configuration Unwinding Plan](bmc-read-only-unwinding-plan.md) is written for BMC only | Create read-only discovery catalog: which datasets, commands, jobs, and queries collect each surface, for BMC first, then Broadcom CA and Rocket | BMC exit criterion (met): first read-only unwinding plan exists for BMC pilot products. Cross-vendor exit criterion (open): equivalent unwinding plans exist for Broadcom CA and Rocket, and at least two pilot products across any vendor have moved out of `unknown_pending_discovery` | Critical |
